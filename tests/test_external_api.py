...
def mock_api() -> Generator[Mock, None, None]:
    """Фикстура для мокирования запросов к API."""
    with patch('src.external_api.requests.get') as mock_get:
        yield mock_get


def test_convert_rub_no_conversion() -> None:
    """Тестирует конвертацию RUB в RUB (без реальной конвертации)."""
...
def test_convert_usd_to_rub(mock_api: Mock) -> None:
    """Тестирует конвертацию USD в RUB с мокированным API."""
    mock_response = Mock()
...
def test_convert_invalid_amount(mock_api: Mock) -> None:
    """Тестирует обработку невалидной суммы."""
...
def test_convert_missing_api_key() -> None:
    """Тестирует обработку отсутствия API ключа."""
...
def test_real_api_conversion() -> None:
    """Тестирует реальное API (пропускается если API недоступно)."""
    try:
